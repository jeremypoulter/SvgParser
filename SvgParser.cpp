...
#if defined(ESP8266) || defined(ESP32) || defined(COMPAT_TEST)
unsigned int
...
uint8_t SvgParser::linkManagement(uint8_t cleanup){
    ...
    if(cleanup){
      DBG_OUT("cleanup done (%p, %p, %p)\n", this, &_linkList, &_linkRefList);
      _linkList = NULL;
      _linkRefList = NULL;
    }
}
...
uint8_t SvgParser::readFlash(const uint8_t *data, size_t size)
{
    // clean up first
    DBG_OUT("Clean up start (%p)\n", this);
    cleanup();
    DBG_OUT("Clean up done (%p)\n", this);
    ...
    //#ifdef ARDUINO
    //memcpy_P(_data, data, size);
    //#else
    memcpy(_data, data, size);
    //#endif
    _data[size] = 0; // terminate
    _bufLen = size;
